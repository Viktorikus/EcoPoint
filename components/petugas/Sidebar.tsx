...
export default function PetugasSidebar() {
  ...
  const petugas = (session as any)?.user

  const isActive = (path: string) => pathname === path || pathname.startsWith(path + "/")

  const menuItems = [
    { label: "Dashboard", icon: "🏠", path: "/petugas/dashboard" },
    { label: "Verifikasi Setoran", icon: "✅", path: "/petugas/verify" },
    { label: "Penjemputan (Pickup)", icon: "🚚", path: "/petugas/pickup" },
  ]
  ...
  return (
    <div className={`${jakarta.className} w-64 bg-white border-r border-gray-200 min-h-screen flex flex-col shadow-sm`}>
      ...
      <div className="p-6 border-b border-gray-200">
        ...
        <p className="text-xs text-gray-600">Petugas Panel</p>
      </div>
      ...
      <div className="p-4 border-t border-gray-200 space-y-3">
        <div className="bg-gray-50 rounded-lg p-3">
          <p className="text-xs text-gray-600 mb-1">Petugas</p>
          <p className="text-sm font-semibold text-gray-900">{petugas?.name || "Petugas"}</p>
          <p className="text-xs text-gray-600">{petugas?.role || "PETUGAS"}</p>
        </div>
        <button
